...
def tui(device: Controller):
    root = py_cui.PyCUI(30, 10)
    root.set_title("Bimatrix controller")
    root.set_refresh_timeout(1)
    info_list = [
        "Current range: {}, Voltage: {}, Mode: {}".format(
            device.current_range, device.voltage, device.mode),
        "DC/DC Converter: {}".format(
            "On" if device.pulse_generator_dc_converter_status else "Off"),
        "Pulse generation: {}".format(
            "On" if device.pulse_generator_dc_converter_status else "Off"),
        "Number of n-plets: {}".format(
            device.num_nplets if device.num_nplets != 0 else "0 (Infinite)"),
        "Time between pulses: {}ms".format(device.time_between),
        "N-plet repetition rate: {}pps".format(device.repetition_rate),
        "Delay after trigger: {}ms".format(device.delay),
        "Pulse widths: {} (unit: μs)".format(device.pulse_widths),
        "Pulse amplitudes: {} (unit: mA)".format(
            calculate_amplitudes(device.pulse_amplitudes, device.mode)),
        "Output channels (unipolar): {}".format(device.output_channels),
        "Channel pairs (bipolar): {}".format(device.channel_pairs),
    ]
    #info = root.add_scroll_menu("Device info", 0, 0, row_span=5, column_span=10)
    #info.add_item_list(info_list)
    #info.set_selectable(False)

    info_list.append("Test elements")

    """span = 10
    curr_range =root.add_block_label("Current range: {}, Voltage: {}, Mode: {}".format(
        device.current_range, device.voltage, device.mode),
        0, 0, column_span=span, center=False)
    converter = root.add_block_label("DC/DC Converter: {}".format(
        "On" if device.pulse_generator_dc_converter_status else "Off"),
        1, 0, column_span=span, center=False)
    root.add_block_label("Pulse generation: {}".format(
        "On" if device.pulse_generator_dc_converter_status else "Off"),
        2, 0, column_span=span, center=False)
    root.add_block_label("Number of n-plets: {}".format(
        device.num_nplets if device.num_nplets != 0 else "0 (Infinite)"),
        3, 0, column_span=span, center=False)
    root.add_block_label("Time between pulses: {}ms".format(device.time_between),
                         4, 0, column_span=span, center=False)
    root.add_block_label("N-plet repetition rate: {}pps".format(device.repetition_rate),
                         5, 0, column_span=span, center=False)
    root.add_block_label("Delay after trigger: {}ms".format(device.delay),
                         6, 0, column_span=span, center=False)
    root.add_block_label("Pulse widths: {} (unit: μs)".format(device.pulse_widths),
                         7, 0, column_span=span, center=False)
    root.add_block_label("Pulse amplitudes: {} (unit: mA)".format(
        calculate_amplitudes(device.pulse_amplitudes, device.mode)),
                         8, 0, column_span=span, center=False)
    root.add_block_label("Output channels (unipolar): {}".format(device.output_channels),
                         9, 0, column_span=span, center=False)
    root.add_block_label("Channel pairs (bipolar): {}".format(device.channel_pairs),
                         10, 0, column_span=span, center=False)
    battery = root.add_block_label("Battery: {}%". format(device.battery_state), 11, 0, column_span=span, center=False)

    command_list1 = root.add_scroll_menu("Commands", 12, 0, row_span=9, column_span=5)
    command_list2 = root.add_scroll_menu("Commands", 12, 5, row_span=9, column_span=5)
    command_history = root.add_scroll_menu("Command history", 21, 0, row_span=8, column_span=span)
    commands = root.add_text_box("Command: ", 29, 0, column_span=10)"""
