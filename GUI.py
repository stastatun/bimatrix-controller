...
from PyQt6.QtCore import Qt, QRectF, QEventLoop, QEvent, QObject, QThread
import sys
...
class Channel(QGraphicsEllipseItem):
    ...
    def mousePressEvent(self, *args, **kwargs):
        if not self.on:
            ...
            self.on = True
            self.scene().views()[0].active_channels.append(self.channel_n)
        else:
            self.setBrush(QColor(0, 0, 255))
            self.on = False
            self.scene().views()[0].active_channels.remove(self.channel_n)
...
class ChannelSwipe(QWidget):
    ...
    def trigger_sweep(self):
        ...
        If self.between is selected, just does the run
        ...
        if not pairs:
            raise Expection("Please select at least one electrode pair")
        for pair in pairs:
    ...
    def generate_combinations(self):
        pairs = []
        actives = self.channels.get_active_channels()
